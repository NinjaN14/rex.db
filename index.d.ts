declare module 'quick.db' {
    ...
    /**
     * This function initiates the database.
     * @param {name} name database name
     * @returns {Database} SQL Database
     */
    function init(name?: string): any;
    ...
    /**
     * This function can be used to perform calculations such as add, subtract etc.
     * @param {key} input any string as a key. Also allows for dot notation following the key.
     * @param {type} type calculation type. Must be one of +, -, *, / or %.
     * @param {options} [input={ target: null }] Any options to be added to the request.
     * @returns {data} the updated data.
     */
    function math(key: string, type: string, value: number, ops?: object): any;
