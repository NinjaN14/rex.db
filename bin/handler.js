...
var methods = {
  ...
  mul: require('../lib/mul.js'),
  div: require('../lib/div.js'),
  mod: require('../lib/sub.js'),
 ...
 * @returns {Database} SQL database
 */
  init: function(name="./json") {
 ...
 */
 ...
 /**
 * This option can be used to do some calculations. Example: add, subtract, multiply etc.
 * @param {key} input any string as a key. Also allows for dot notation following the key.
 * @param {options} [input={ target: null }] Any options to be added to the request.
 * @returns {data} the updated data.
 */
  
  math: function(key, type, value, ops) {
    let valid = ["+", "-", "/", "*", "%"];
    if (!key) throw new TypeError('No key specified. Need Help? Check Out: discord.gg/plexidev');
    if (!type) throw new TypeError('Must specify the type: +, -, *, / or %');
    if (!valid.includes(type)) throw new TypeError('Type must be one of ' + valid.join(", "));
    if (isNaN(value)) throw new TypeError('Must specify value. Need Help? Check Out: discord.gg/plexidev');
    switch (type) {
        case "+":
            arbitrate('add', {id: key, data: value, ops: ops || {}});
            break;
        case "-":
            arbitrate('sub', {id: key, data: value, ops: ops || {}});
            break;
        case "/":
            arbitrate('div', {id: key, data: value, ops: ops || {}});
            break;
        case "*":
            arbitrate('mul', {id: key, data: value, ops: ops || {}});
            break;
        case "%":
            arbitrate('mod', {id: key, data: value, ops: ops || {}});
            break;
    };
  },
 ...
 */
 ...
 /**
 * This function deletes the entire active table.
 * @returns {boolean}
 */
  
  deleteAll: function() { 
    return arbitrate('deleteAll', {ops: {}});
  },
 ...
 */
  ...
  table: function(tableName, options = {}) {
    ...
    this.math = function(key, type, value, ops) {
      let valid = ["+", "-", "/", "*", "%"];
      if (!key) throw new TypeError('No key specified. Need Help? Check Out: discord.gg/plexidev');
      if (!type) throw new TypeError('Must specify the type: +, -, *, / or %');
      if (!valid.includes(type)) throw new TypeError('Type must be one of ' + valid.join(", "));
      ...
      switch (type) {
        case "+":
          arbitrate('add', {id: key, data: value, ops: ops || {}}, this.tableName);
          break;
      case "-":
          arbitrate('sub', {id: key, data: value, ops: ops || {}}, this.tableName);
          break;
      case "/":
          arbitrate('div', {id: key, data: value, ops: ops || {}}, this.tableName);
          break;
      case "*":
          arbitrate('mul', {id: key, data: value, ops: ops || {}}, this.tableName);
          break;
      case "%":
          arbitrate('mod', {id: key, data: value, ops: ops || {}}, this.tableName);
          break;
      };
    }
    ...
    this.table = function(tableName, options = {}) {
      return new this(tableName, options={});
    }
